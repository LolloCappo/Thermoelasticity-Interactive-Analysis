...
if flag:
    ...
    cordinate = RoiSelect.selectROI(Analisi.finestra_video[:,:,0],titolo='Seleziona Area Analisi')
...
cordinate = RoiSelect.selectROI(Analisi.finestra_video[:,:,0],titolo='Seleziona ROI')
Analisi.selezione_ROI(cordinate[1],cordinate[0],cordinate[3],cordinate[2],view = True)


cordinate = RoiSelect.selectROI_ellipse(Analisi.finestra_video_roi_media,titolo='Seleziona ROI foro')
Analisi.selezione_ROI_foro(cordinate[1],cordinate[0],cordinate[3],cordinate[2])

cordinate = RoiSelect.selectROI(Analisi.finestra_video_roi_media,titolo='Seleziona finestra di compensazione')
print(cordinate)
Analisi.selezione_ROI_compensazione(cordinate[1],cordinate[0],cordinate[3],cordinate[2],fr,df = 5,view=True)
...
(mappa_modulo,mappa_fase)=Analisi.ottieni_mappe_lockin(view = True,t_lim_inf = None, t_lim_sup = None)
#Analisi.ottieni_mappe_lockin(fr=fr,view = True,t_lim_inf = None, t_lim_sup = None)
...
l = plt.imshow(np.abs(np.abs(mappa_fase)-np.mean(mappa_fase)*np.ones(mappa_fase.shape)),cmap='magma')
...
cmap_lim_inf = Slider(ax_lim_inf, 'sunf', np.min(mappa_fase), np.max(mappa_fase), valinit=lim_inf)
cmap_lim_sup = Slider(ax_lim_sup, 'sup', np.min(mappa_fase), np.max(mappa_fase), valinit=lim_sup)
